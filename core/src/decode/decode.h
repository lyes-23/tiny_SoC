...
#include "fifo_element.h"
  ...
  //==========================================
    ...
    sc_out<sc_uint<32>>	id2exe_mem_data;

    sc_out<bool>		    id2exe_jump;

    sc_out<bool>		    id2exe_branch;
    sc_out<sc_uint<32>> id2exe_branch_offset;

    sc_out<sc_uint<4>>	id2exe_alu_ctrl;
    ...
    sc_signal<sc_uint<32>> branch_offset;
    ...
    sc_signal<sc_uint<4>>	alu_ctrl;
    sc_signal<bool>         signed_op;


    // pipeline signals
    sc_signal<sc_bv<ID2EXE_SIZE>>	in_id2exe_data;
    ...
    sc_signal<bool>			id2exe_empty;
     ...
     sc_signal<bool> fence_type_inst; // NOP type format 
     ...
     void trace(sc_trace_file *tf);
      ...
      SC_CTOR(decode) : id2exe("id2exe"){
        ...
        id2exe.out_data_o	  (out_id2exe_data);
        id2exe.full_o	      (id2exe_full);
        id2exe.empty_o	    (id2exe_empty);


        SC_METHOD(concat_id2exe);
        sensitive << result_src << alu_ctrl << mem_size << reg_wr_en << mem_wr_en 
                  << mem_data << jump << branch << imm_ext << reg_dest 
                  << reg_src1 << reg_src2 << rdata1 << rdata2 << branch_offset 
                  << if2id_pc_next << out_id2exe_ready;

        SC_METHOD(unconcat_id2exe);
        sensitive << out_id2exe_data;

        SC_METHOD(decoding_inst_type);
        sensitive << if2id_inst;

        SC_METHOD(decoding_inst);
        sensitive << if2id_inst << op_type;
